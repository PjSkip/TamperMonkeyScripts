...
(function () {
  ...
  function firstQuoteBlock(root) {
    if (!root || !root.querySelector) return null;
    try {
      var direct = root.querySelector(
        ':scope > .gmail_quote, :scope > .gmail_extra, :scope > blockquote.gmail_quote'
      );
      if (direct) return direct;
    } catch (eQ) {}
  ...
  function firstQuoteAuthor(msg) {
    ...
    if (!q) return '';
    var attr =
      (q.querySelector && q.querySelector('.gmail_attr')) ||
      null;
    if (!attr) {
      var prev = q.previousElementSibling;
      if (prev && /(^|\s)gmail_attr(\s|$)/.test(prev.className || '')) attr = prev;
    }
    var who = addrFromQuoteAttr(attr) || quoteAuthorAddr(q);
    if (who && isSkipCarrierAddr(who)) return '';
    return who;
  }
  function quotedMcsFromMessage(msg) {
    var seen = {};
    var out = [];
    function add(mc) {
      if (!mc || seen[mc] || shouldIgnore(mc)) return;
    ...
    function walk(node, isRoot) {
      if (!node || t.length >= 2500) return;
      if (node.nodeType === 1) {
        var cls = node.className ? String(node.className) : '';
        if (!isRoot && /gmail_quote|gmail_extra/.test(cls)) return;
        var kids = node.childNodes;
        var i;
        for (i = 0; i < kids.length; i++) walk(kids[i], false);
        return;
      }
      if (node.nodeType === 3 && node.nodeValue) t += node.nodeValue + ' ';
    }
    walk(q, true);
    var hits = findMcMatches(t.slice(0, 2500));
    var h;
    for (h = 0; h < hits.length; h++) add(hits[h].mc);
    return out;
  }
  function assignedMcsForTeamMessage(msg, tid) {
    var quoted = quotedMcsFromMessage(msg);
    if (quoted.length) return quoted;
    var author = firstQuoteAuthor(msg);
    if (author) {
      var fromQuote = assignedMcsForAddr(author, tid);
      if (fromQuote.length) return fromQuote;
    }
    var recips = nonTeamRecipients(msg);
    if (recips.length === 1) return assignedMcsForAddr(recips[0], tid);
    var extra = [];
    var i;
    for (i = 0; i < recips.length; i++) {
      extra = extra.concat(assignedMcsForAddr(recips[i], tid));
    }
    return uniqMcs(extra);
  }
  ...
  function barMcsForMessage(msg) {
    ...
    if (isSelfOrCoworkerAddr(from)) return uniqMcs(own.concat(assignedMcsForTeamMessage(msg, tid)));
    return uniqMcs(own.concat(assignedMcsForAddr(from, tid)).concat(quotedMcsFromMessage(msg)));
  }
  function learnMcsFromThread(root) {
    ...
    for (i = 0; i < msgs.length; i++) {
      ...
      var quotedMcs = quotedMcsFromMessage(msg);
      var quoteWho = firstQuoteAuthor(msg);
      if (quoteWho) {
        for (u = 0; u < quotedMcs.length; u++) addMap(fromQuoted, quoteWho, quotedMcs[u]);
      }
      if (isSelfOrCoworkerAddr(from) && unq.length) {
        var recips = nonTeamRecipients(msg);
        if (recips.length === 1) {
          for (u = 0; u < unq.length; u++) addMap(fromUnquoted, recips[0], unq[u]);
        } else if (quoteWho && quotedMcs.length) {
          for (u = 0; u < quotedMcs.length; u++) addMap(fromUnquoted, quoteWho, quotedMcs[u]);
        }
      }
    }
  ...
  function collectTextNodes(root) {
    ...
    function walk(node, isRoot) {
      ...
      if (node.nodeType === 1) {
        var cls = node.className ? String(node.className) : '';
        if (!isRoot && SKIP_WALK.test(cls)) return;
  ...
  function messageMcForRates(root) {
    ...
    var assigned = isSelfOrCoworkerAddr(from)
      ? assignedMcsForTeamMessage(msg, tid)
